...
function Alias(){
    ...
    function AddAlias(el){
        el.preventDefault()
        console.log()
        dispatch(aliasAddItems(el.target.parentNode.children[1].textContent))
    }

    function AddMore(el){
        const name = el.target.parentNode.parentNode.children[1].textContent
        dispatch(AddMoreQuantityToItemsCreator(name))
    }
    function Addless(el){
        const name = el.target.parentNode.parentNode.children[1].textContent
            ...
            <div className="grid grid-3-col gap16">
                {aliasHome.map(el=>
                    <div key={el._id} className='cover flex flex-2 flex-dir gap8'>
                        {/* {el.name==='baigan long' ?
                            <img src={baigan} className='iconeg' alt='vegeable'/>
                         :  
                        ...
                        <p className='namealias'>{el.name}</p>
                        {el.addedValue === 0 || el.addedValue === '0' ?
                            <button onClick={AddAlias} className='addtocart'>Add+</button>
                        :   
                        <div className='addless flex flex-1 gap8'>
                            <button onClick={AddMore} className=' addBtn'>+</button>
                            <p className='addedunits'>{el.addedValue}{el.units}</p>
                            <button onClick={Addless} className=' addBtn'>-</button>
                        </div>
                        }
                    </div>
                )}
